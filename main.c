...
int main(int argc, char **argv)
{
    ...
    vkGetPhysicalDeviceSurfaceSupportKHR(*phy_best_devi, qf_best_index,
    ...
    if (surf_caps.currentExtent.width != wind_w || surf_caps.currentExtent.height != wind_h)
    {
        extent_suitable = 0;
    ...
    for (uint32_t i = 0; i < swap_image_count; i++)
    {
        ...
        vkCreateFramebuffer(dev, &(frame_buff_cre_infos[i]),
                            NULL, &(frame_buffs[i]));
    ...
    for (uint32_t i = 0; i < swap_image_count; i++)
    {
        ...
        vkBeginCommandBuffer(cmd_buffers[i],
                             &cmd_buff_begin_infos[i]);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        vkWaitForFences(dev, 1, &(fens[cur_frame]),
                        VK_TRUE, UINT64_MAX);
        ...
        {
            vkWaitForFences(dev, 1, &(fens_img[img_index]),
                            VK_TRUE, UINT64_MAX);
        }
    ...
    vkFreeCommandBuffers(dev, cmd_pool,
